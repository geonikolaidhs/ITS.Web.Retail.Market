...
namespace ITS.Retail.Platform.Tests.POS
{
    public class MockSessionManager : ISessionManager
    {
        ...
        public void ClearAllSessions()
        {
            ...
            foreach(KeyValuePair<Assembly, IDataLayer> pair in MemoryXpoHelper.AllActiveDataLayers.ToList())
            {
                pair.Value.Dispose();
                MemoryXpoHelper.AllActiveDataLayers.Remove(pair.Key);
            }
        }
        ...
        {
            return MemoryXpoHelper.GetNewUnitOfWork(type);
        }
    }
}
