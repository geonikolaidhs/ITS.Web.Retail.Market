...
namespace ITS.POS.Client.Actions
{
    ...
    /// </summary>
    public class ActionDeletePayment : Action
    {

        public ActionDeletePayment(IPosKernel kernel) : base(kernel)
        {
        ...
        {
            DocumentPayment payment = (parameters as ActionDeletePaymentParams).Payment;
                ...
                if (formManager.ShowMessageBox(POSClientResources.DO_YOU_WANT_TO_CANCEL_LINE, MessageBoxButtons.OKCancel) == DialogResult.OK)
                {
                    if (payment.DocumentPaymentEdps != null)
                    {
                        ...
                        if (device != null)
                        {
                            using (frmMessageBox dialog = formManager.CreateMessageBox(POSClientResources.PROCESS_PAYMENT_ON_DEVICE))
                            {
                                ...
                                {
                                    result = device.CancelPayment(edpsPayment.ReceiptNumber, Math.Abs(payment.Amount), 0, 1, 0, config.TerminalID.ToString());
                                }
                                else
                                {
                                ...
                                else
                                {
                                    ...
                                    if (result != null && !string.IsNullOrWhiteSpace(result.ErrorCode))
                                    {
                    ...
                    if (payment.DocumentPaymentCardlink != null)
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            throw new POSUserVisibleException(ex.Message.ToString());
                        }
                    }
                    ...
                    .Where(transCoupon =>
                            ...
                            && transCoupon.DocumentPayment.Oid == payment.Oid
                          )
                    .FirstOrDefault();
