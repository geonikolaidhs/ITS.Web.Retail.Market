...
{
    /// <summary>
    /// Display's the selected document detail's discounts
    /// </summary>
    public class ucDiscountsDataGrid : ucDataGridViewExtension, IObserverDocumentDetailDisplayer
    {

        public ucDiscountsDataGrid()
        {
        ...
        {
            if (parameters.Navigation != eNavigation.MOVEDOWN && parameters.Navigation != eNavigation.MOVEUP)
            {
        ...
        public void Update(DocumentDetailDisplayerParams parameters)
        {
            ...
            DataSource = parameters.DocumentDetail.DocumentDetailDiscounts.Where(x => x.DiscountSource != eDiscountSource.DOCUMENT && x.DiscountSource != eDiscountSource.DEFAULT_DOCUMENT_DISCOUNT
                                                                                    && x.DiscountSource != eDiscountSource.CUSTOMER).Select(x => DocumentDetailDiscountHelper.Convert(x, sessionManager, platformRoundingHandler)).ToList();
        }
