...
namespace POSLoader.WebServiceAutoGeneration
{
    public class WebServiceList
    {
        ...
        {
            XmlTextWriter xml = new XmlTextWriter(path, null) { Formatting = Formatting.Indented };
        ...
        public static WebServiceList Deserialize(string path)
        {
            ...
            foreach (XmlElement e in doc.GetElementsByTagName("service"))
            {
                ...
                foreach (XmlElement m in e.GetElementsByTagName("method"))
                {
                    ...
                    method.SetArg(ser.Deserialize(new System.IO.StringReader(m.InnerXml)));
                }
            }
        ...
        public WebMethod FindMethod(String wmName)
        {
            foreach (WebService svc in Services)
            {
                WebMethod wm = svc.FindMethod(wmName);
                if (wm != null)
                    return wm;
            }
            return null;
