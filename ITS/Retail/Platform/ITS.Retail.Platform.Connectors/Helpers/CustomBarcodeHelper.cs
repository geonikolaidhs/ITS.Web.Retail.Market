...
namespace ITS.Retail.Platform.Common.Helpers
{
    public static class CustomBarcodeHelper
    {
        ...
        public static BarcodeParseResult ParseCustomBarcode<T>(List<T> barcodeTypes, string scannedBarcode, bool doPadding, int barcodeLength,
            char paddingChar, bool padRight = true) where T : XPBaseObject
        {
        ...
        private static Guid? GetBarcodeTypeOid<T>(T barcodeType) where T : XPBaseObject
        {
        ...
        /// <summary>
        /// Get relevant BarcodeTypes matching given code for type basicObjectType (FullName).Use this ONLY for POS!!!
        /// </summary>
        /// <param name="code">Given Code</param>
        /// <param name="basicObjectType">Given Model Type.The table to search into.Given Model FullName will be translated into web Model namespace. If this is string.Empty then this is not used to filter BarcodeTypes.</param>
        /// <returns></returns>
        public static List<ITS.POS.Model.Settings.BarcodeType> GetMatchingMasks(string code, string basicObjectType, UnitOfWork uow)
        {
            basicObjectType = basicObjectType.Replace("ITS.POS.Model.Master", "ITS.Retail.Model").Replace("ITS.POS.Model.Settings", "ITS.Retail.Model");
            CriteriaOperator criteria = string.IsNullOrEmpty(basicObjectType) ? null : new BinaryOperator("EntityType", basicObjectType);

            return new XPCollection<ITS.POS.Model.Settings.BarcodeType>(uow, criteria)
                    .Where(barType => barType.Length == code.Length
                            && code.StartsWith(barType.Prefix)
                            && NonSpecialCharactersMatch(code, barType.Prefix + barType.Mask))
                    .ToList();
        }
    }
}
