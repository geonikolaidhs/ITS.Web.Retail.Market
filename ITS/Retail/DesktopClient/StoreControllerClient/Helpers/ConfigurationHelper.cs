using ITS.Retail.DesktopClient.StoreControllerClient.Interfaces;
using System;
    ...
    public static class ConfigurationHelper
    {
            ...
            try
            {
                ...
                if (nodelist.Count == 1)
                {
                    ...
                    foreach (PropertyInfo propInfo in properties)
                    {
                        ...
                        {
                            object value = el[propInfo.Name].InnerText.Trim();
        ...
        private static void Assign<T>(T obj, XmlElement el)
        {
            foreach (PropertyInfo propInfo in typeof(T).GetProperties().Where(property => property.CanWrite))
            {
                if (el[propInfo.Name] != null)
                {
                    ...
                    {
                        value = Enum.Parse(propInfo.PropertyType, el[propInfo.Name].InnerText.Trim());
                    }
                    else if (propInfo.PropertyType.Equals(typeof(char)))
                    {
                        string xmlValueString = el[propInfo.Name].InnerText.Trim();
        ...
        private static void CreateSubElementStatic(Type type, XmlDocument xmlDoc, XmlElement root)
        {
            foreach (PropertyInfo propInfo in type.GetProperties())
            {
                ...
                {
                    object val = propInfo.GetValue(null, null);
                    node.InnerText = val == null ? "" : val.ToString();
                }
                root.AppendChild(node);
            }
        }
