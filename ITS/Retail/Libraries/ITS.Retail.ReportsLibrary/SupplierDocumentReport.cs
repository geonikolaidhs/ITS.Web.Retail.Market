...
{
    public partial class SupplierDocumentReport : DevExpress.XtraReports.UI.XtraReport
    {
        protected SupplierDocumentReport()
        {
        }


        protected User RequestedUser { get; set; }
        public SupplierDocumentReport(User requestUser)
        {
            InitializeComponent();
        ...
        private void DocumentReport_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            xrLabelTaxCodeOffice.Text = Resources.TaxCode.ToUpper() + ":" + (documentHeader.Store.Owner.Trader.TaxCode ?? "") + ", " + Resources.TaxOffice.ToUpper() + ":" + OwnerTaxOfficeDescription;
            ...
            xrLabelCustCompanyName2.Text = documentHeader.Customer != null ? documentHeader.Customer.CompanyName : "";
            ...
            xrLabelCustCode.Text = documentHeader.Customer!=null ? documentHeader.Customer.Code : "";
            xrLabelCustCompanyName.Text = documentHeader.Customer != null ? documentHeader.Customer.CompanyName : "";
            xrLabelCustProfession.Text = documentHeader.Customer!=null ?  documentHeader.Customer.Profession : "";
            if (documentHeader.Customer != null && documentHeader.Customer.DefaultAddress == null)
            {
                try
                {
                    xrLabelCustAddress.Text = documentHeader.Customer!=null && documentHeader.Customer.Trader!=null ? documentHeader.Customer.Trader.Addresses[0].Street : "";
                    Address address = documentHeader.Customer != null && documentHeader.Customer.Trader != null && documentHeader.Customer.Trader.Addresses.Count > 0 ? documentHeader.Customer.Trader.Addresses[0] : null;
                    
                    string city = address!=null ? address.City : "";
                    string postCode = address != null ? address.PostCode : "";
                    xrLabelCustCityPOCode.Text = city + "/" + postCode;
                    if (address!=null && address.DefaultPhone == null)
                    {
                        try
                        {
                            xrLabelCustPhoneNumber.Text = address.Phones[0].Number ?? "";
                        }
                        catch
                        {
            ...
            {
                Address defaultAddress = documentHeader.Customer != null ? documentHeader.Customer.DefaultAddress : null;
                string city = defaultAddress != null ? defaultAddress.City : "";
                string postCode = defaultAddress != null ? defaultAddress.PostCode : "";
                xrLabelCustAddress.Text = defaultAddress!=null ? defaultAddress.Street : "";
                xrLabelCustCityPOCode.Text = city + " " + postCode;
                if ( defaultAddress!=null && defaultAddress.DefaultPhone == null)
                {
                    try
                    {
                        xrLabelCustPhoneNumber.Text = defaultAddress.Phones[0].Number ?? "";
                    }
                    catch
                    {
                        xrLabelCustPhoneNumber.Text = "";
            ...
            xrLabelCustTaxCode.Text = documentHeader.Customer != null && documentHeader.Customer.Trader != null ? documentHeader.Customer.Trader.TaxCode : "";
            ...
            CriteriaOperator visibleDetails = null;
			...
			else if (user.IsCentralStore)
			{
				visibleDetails = CriteriaOperator.And(new NotOperator(new NullOperator("CentralStore")), new BinaryOperator("CentralStore.Oid", ((XPCollection<DocumentHeader>)DataSource)[0].Store.Oid, BinaryOperatorType.NotEqual));
			}
			else
			{
				visibleDetails = CriteriaOperator.Or(new NullOperator("CentralStore"), new BinaryOperator("CentralStore.IsCentralStore", false));
			}

            ((XPCollection<DocumentHeader>)DataSource)[0].DocumentDetails.Filter = visibleDetails;
            var detailsWithCategoryAndStorage = from detail in ((XPCollection<DocumentHeader>)DataSource)[0].DocumentDetails
                                                select new
                                                {
                                                    itemCategory = detail.Item.ItemAnalyticTrees.FirstOrDefault() != null ?
                                                                       ...
                                                                       : "")
                                                                       : "",
                                                    itemCode = detail.DocumentHeader.Owner.OwnerApplicationSettings.PadItemCodes  ? detail.Item.Code.TrimStart(detail.DocumentHeader.Owner.OwnerApplicationSettings.ItemCodePaddingCharacter[0]) : detail.Item.Code,
                                                    itemCodeForSorting = detail.Item.Code,
                                                    itemStorage = (detail.CentralStore != null ? detail.CentralStore.Code : ""),
            ...
            DetailReport.DataSource = detailsWithCategoryAndStorage.OrderBy(x => x.itemCodeForSorting).OrderBy(y => y.itemCategory);

            xrTableCellItemCategory.DataBindings.Add("Text", DetailReport.DataSource, "itemCategory");
        ...
        private void xrTableCellStaticVatTotal_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
